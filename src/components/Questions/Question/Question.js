...
export default function Question({ question, activeRubrics }) {
  ...
  return (
    <article className={`${styles.question}${activeRubrics.length > 0 && question.tags.every((tag) => activeRubrics.indexOf(tag) === -1) ? ' display_none' : ''}`}>
      ...
      <div className={styles.question__wrap}>
        {
          question.tags.map((tag, i) => <p key={i.toString()} className={`${styles.rubric} ${styles.question__rubric}`}>{tag.name}</p>)
        }
